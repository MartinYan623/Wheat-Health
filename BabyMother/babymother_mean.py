...
import datetime
encoding='UTF-8'
data=pd.read_csv('/Users/martin_yan/Desktop/1.csv')
info=pd.read_csv('../data/宝妈用户减重表5.22-6.18(全部).csv',usecols=[1,2])
...
for i in range(len(data)):
    ...
    if username[i]==False:
        ...
        columns = ['用户编号', '姓名', '年龄','BMI','记录天数','水果实际摄入平均量', '水果摄入量平均分', '蔬菜实际摄入平均量', '蔬菜摄入量平均分', '全谷类实际摄入平均量', '全谷类摄入量平均分',
                    ...
                    '添加糖实际摄入平均量', '添加糖摄入量平均分', '总热量摄入量平均分', '三大营养素实际摄入平均量', '三大营养素组成平均分', '饮酒实际摄入平均量',
...
data=pd.read_csv('/Users/martin_yan/Desktop/每日热量统计.csv',usecols=[1,5,6])
...
for i in range(len(data)):
    ...
    if last[i] == False:
        ...
        #除以实际入营天数
        meanlight.append(sum_light / 8)
        meannormal.append(sum_normal / 20)
...
meanheat=pd.DataFrame({'姓名':name,'轻食日热量平均对比值':meanlight,'普通日热量平均对比值':meannormal})
