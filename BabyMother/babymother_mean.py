...
info=pd.read_csv('../data/宝妈用户减重表5.22-6.4.csv',usecols=[1,2])
...
for i in range(len(data)):
    ...
    if username[i]==False:
        ...
        dataframe = pd.DataFrame({'用户编号': [data.iloc[i][0]], '姓名': [data.iloc[i][2]], '记录天数': [count],
                                   ...
                                   '平均得分': [score / count],'减重值':[data.iloc[i][37]],'初始体重值':[data.iloc[i][38]],
                                   '减重百分比':[data.iloc[i][37]/data.iloc[i][38]],'BMI':[data.iloc[i][39]],'年龄':[data.iloc[i][40]]})
        """
        #除以入营以来的天数
        dataframe = pd.DataFrame({'用户编号': [data.iloc[i][0]], '姓名': [data.iloc[i][2]], '记录天数': [count],
                                   '水果实际摄入平均量': [sum_f_fruit / 14], '水果摄入量平均分': [sum_fruit / 14],
                                   '蔬菜实际摄入平均量': [sum_f_veg / 14], '蔬菜摄入量平均分': [sum_veg /14],
                                   '全谷类实际摄入平均量': [sum_f_wholegrain / 14],
                                   '全谷类摄入量平均分': [sum_wholegrain / 14],
                                   '精制谷物摄入平均量': [sum_f_refinegrain / 14],
                                   '精制谷物摄入量平均分': [sum_refinegrain / 14],
                                   '膳食纤维实际摄入平均量': [sum_f_diefiber / 14],
                                   '膳食纤维摄入量平均分': [sum_diefiber / 14],
                                   '乳类实际摄入平均量': [sum_f_milk / count], '乳类摄入量平均分': [sum_milk / 14],
                                   '总蛋白实际摄入平均量': [sum_f_totalprotein / 14],
                                   '总蛋白摄入量平均分': [sum_totalprotein / 14],
                                   '鱼虾贝壳类及植物蛋白类实际摄入平均量': [sum_f_fishshrimp / 14],
                                   '鱼虾贝壳类及植物蛋白类摄入量平均分': [sum_fishshrimp / 14],
                                   '不饱和与饱和脂肪酸实际摄入平均量': [sum_f_fattyacids / 14],
                                   '不饱和与饱和脂肪酸摄入比平均分': [sum_fattyacids / 14],
                                   '固态脂肪实际摄入平均量': [sum_f_solidfat / 14],
                                   '固态脂肪摄入量平均分': [sum_solidfat / 14],
                                   '钠盐实际摄入平均量': [sum_f_salt / 14], '钠盐摄入量平均分': [sum_salt / 14],
                                   '添加糖实际摄入平均量': [sum_f_sugar / 14], '添加糖摄入量平均分': [sum_sugar /14],
                                   '总热量实际摄入平均量': [sum_f_totalheat / 14],
                                   '总热量摄入量平均分': [sum_totalheat / 14],
                                   '三大营养素实际摄入平均量': [sum_f_nutrients / 14],
                                   '三大营养素组成平均分': [sum_nutrients / 14],
                                   '饮酒实际摄入平均量': [sum_f_alcohol / 14],
                                   '饮酒（酒精量，全天标准）平均分': [sum_alcohol / 14],
                                   '饮水平均量': [sum_f_water / 14], '饮水量平均分': [sum_water / 14],
                                   '平均得分': [score / 14], '减重值': [data.iloc[i][37]], '初始体重值': [data.iloc[i][38]],
        ...
        dataframe.to_csv('/Users/martin_yan/Desktop/3.csv', index=False, encoding="utf_8_sig",
