...
import datetime
encoding='UTF-8'
data=pd.read_csv('/Users/martin_yan/Desktop/整理数据5.22-6.25.csv')
data=data[data['记录日期']> '2018/6/18 0:00']
...
info=pd.read_csv('../data/宝妈营数据/宝妈用户减重表5.22-5.28.csv',usecols=[1,2])
...
for i in range(len(data)):
    ...
    if username[i]==False:
        ...
        dataframe2 = pd.DataFrame({'用户编号': [data.iloc[i][0]], '姓名': [data.iloc[i][2]], '记录天数': [count],
                                   ...
                                   'HEI平均得分(100分)': [heiscore / count],
                                   '饮食均衡得分(120分)':[balancescore/count],
                                   '新饮食得分(100制)':[totalscore/count],
        ...
        columns = ['用户编号', '姓名', '年龄', 'BMI', '记录天数', '水果实际摄入平均量', '水果摄入量平均分', '蔬菜实际摄入平均量', '蔬菜摄入量平均分', '全谷类实际摄入平均量',
                   ...
                   '饮酒（酒精量，全天标准）平均分', '饮水平均量', '饮水量平均分', 'HEI平均得分(100分)','饮食均衡得分(120分)','新饮食得分(100制)', '初始体重值', '减重值', '减重百分比']

        dataframe2.to_csv('/Users/martin_yan/Desktop/2.csv', index=False, encoding="utf_8_sig",columns=columns,mode='a',header=sec)
...
columns= ['用户编号', '姓名', '年龄','BMI','记录天数','完整记录天数','水果实际摄入平均量', '水果摄入量平均分', '蔬菜实际摄入平均量', '蔬菜摄入量平均分', '全谷类实际摄入平均量', '全谷类摄入量平均分',
                    ...
                    '饮酒（酒精量，全天标准）平均分', '饮水平均量', '饮水量平均分','HEI平均得分(100分)','饮食均衡得分(120分)','新饮食得分(100制)','初始体重值','减重值','减重百分比','助理名称']
