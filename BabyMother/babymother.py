...
for i in range(len(data)):
    ...
    if data.iloc[i]['rf'] == '总热量摄入量':
        ...
        f_totalheat.append(data.iloc[i]['对比值'])
...
dataframe = pd.DataFrame({'用户编号': userid, '姓名': name, '记录日期': time,
                              ...
                              '总热量摄入对比值': f_totalheat, '总热量摄入量得分': totalheat,
...
columns = ['用户编号', '姓名', '记录日期', '水果摄入实际量', '水果摄入量得分', '蔬菜摄入实际量', '蔬菜摄入量得分', '全谷类摄入实际量', '全谷类摄入量得分', '精制谷物摄入实际量',
        ...
        , '不饱和与饱和脂肪酸实际摄入量', '不饱和与饱和脂肪酸摄入比得分', '固态脂肪摄入实际量', '固态脂肪摄入量得分', '钠盐摄入实际量', '钠盐摄入量得分', '添加糖摄入实际量', '添加糖摄入量得分',
               '总热量摄入对比值', '总热量摄入量得分', '三大营养素摄入实际量', '三大营养素组成得分', '饮酒摄入实际量', '饮酒（酒精量，全天标准）得分', '饮水量', '饮水量得分','总得分']
dataframe.to_csv('/Users/martin_yan/Desktop/babymother_data5.22-5.2812.csv', index=False, encoding="utf_8_sig", columns=columns)
