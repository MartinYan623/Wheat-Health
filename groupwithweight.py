...
for i in range(len(data)):
    if data.iloc[i]['平均得分']>84:
        ...
        if note[i] == False:
            ...
            reduce.append(weight - data.iloc[i]['体重'])
            time.append(str(data.iloc[i - 1]['日期']) + ' ~ ' + str(data.iloc[i]['日期']))
            group.append("B组(平均得分<=84)")
            score.append(data.iloc[i]['平均得分'])
            BMI.append(data.iloc[i]['BMI'])
dataframe = pd.DataFrame({'姓名': username, '组别':group,'减肥时间段': time,'减重值':reduce,'平均得分':score,'BMI':BMI})
columns = ['姓名','组别','减肥时间段','减重值','平均得分','BMI']
print(dataframe)
print(count1)
print(count2)
